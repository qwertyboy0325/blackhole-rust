...
/// Provisional calibration band (Hz) covering IR–UV for Gate 2C0 T_eff range after g-mapping.
...
/// Build an explore physical Hz grid (not frozen; bin count is calibration evidence).
...
pub fn parse_physical_spectral_grid_id(grid_id: &str) -> Result<SpectralGrid, SpectralRenderError> {
    if grid_id == "spectral-grid-v1" || grid_id.starts_with("spectral-grid-explore-") {
        return Err(SpectralRenderError::UnsupportedGridId(format!(
            "diagnostic grid `{grid_id}` is not physical Hz; use `{PHYSICAL_GRID_EXPLORE_PREFIX}{{n}}`"
        )));
    }
    let n: u32 = grid_id
...
pub struct PhysicalSpectralClosureMetrics {
    ...
    pub worst_emitter_pixel: Option<(u32, u32)>,
    pub worst_transport_pixel: Option<(u32, u32)>,
...
fn map_disk_hit(
    ...
    // Truncation: fraction of π∫B outside the emitter-mapped observer grid.
    ...
    let total = planck_mass_on_interval(
        t,
        PHYSICAL_GRID_NU_MIN_HZ / 10.0,
        PHYSICAL_GRID_NU_MAX_HZ * 10.0,
        1024,
    )?;
...
pub fn compute_physical_spectral_closure(
    ...
    let mut worst_em = None;
    let mut worst_tr = None;
    let mut count = 0u64;
    for row in 0..frame.grid.height {
        for col in 0..frame.grid.width {
            ...
            if rel_em > max_rel_em || (rel_em == max_rel_em && abs_em > max_abs_em) {
                max_rel_em = rel_em;
                max_abs_em = abs_em;
                worst_em = Some((col, row));
            }
            ...
            if rel_tr > max_rel_tr || (rel_tr == max_rel_tr && abs_tr > max_abs_tr) {
                max_rel_tr = rel_tr;
                max_abs_tr = abs_tr;
                worst_tr = Some((col, row));
            }
        }
    }
    Ok(PhysicalSpectralClosureMetrics {
        ...
        worst_emitter_pixel: worst_em,
        worst_transport_pixel: worst_tr,
